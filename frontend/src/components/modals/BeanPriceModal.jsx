...
import { useUpdateCoffeeBeanPrice } from "@/hooks/useCoffeeBeans";
...
export function BeanPriceModal({ bean, onClose }) {
  const [price, setPrice] = useState(bean.price ?? "");
  const updatePrice = useUpdateCoffeeBeanPrice();
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {bean.variety ? `${capitalize(bean.variety)} Beans` : "Coffee Beans"}{" "}
              {bean.owner?.fullName && (
